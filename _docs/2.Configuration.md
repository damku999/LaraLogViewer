...
return [
    ...
    'route'         => [
        ...
        'attributes' => [
            ...
            'middleware' => env('WEBMONKS_LARAVEL_LOG_VIEWER_MIDDLEWARE') ? explode(',', env('WEBMONKS_LARAVEL_LOG_VIEWER_MIDDLEWARE')) : null,
        ],
    ],
...
By default no middleware is added to `laravel-log-viewer` route. If you need middlewares you just have to add a `WEBMONKS_LARAVEL_LOG_VIEWER_MIDDLEWARE` key to your `.env` file and add middlewares as comma separated values (no space).

 * **Example 1:** single middleware

```
WEBMONKS_LARAVEL_LOG_VIEWER_MIDDLEWARE=web
```

 * **Example 2:** multiple middlewares
 
```
WEBMONKS_LARAVEL_LOG_VIEWER_MIDDLEWARE=web,auth,custom-middleware
```
